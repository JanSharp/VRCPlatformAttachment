...
namespace JanSharp
{
    ...
    public partial class PlatformAttachmentManager : UdonSharpBehaviour
    {
        ...
        {
            var origin = localPlayer.GetTrackingData(VRCPlayerApi.TrackingDataType.Origin);
            qd.ShowForOneFrame(this, "position", $"{origin.position:f3}");
            qd.ShowForOneFrame(this, "rotation", $"{origin.rotation.eulerAngles:f3}");
        }

        public void TeleportPlayer(Vector3 position, Quaternion rotation)
        {
            PrintOrigin();
            localStation.PlayerMobility = VRCStation.Mobility.Immobilize;
            PrintOrigin();
            localStationPlayerPosition.SetPositionAndRotation(position, rotation);
            PrintOrigin();
            localStation.PlayerMobility = VRCStation.Mobility.Mobile;
            PrintOrigin();
